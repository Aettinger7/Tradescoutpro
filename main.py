...
# Top coins + requested Base memecoins
COINS = [
...
def index():
 ...
 change_sign = "+" if coin["change_24 h"] > 0 else ""
 mcap = f"${coin['market_cap']:,.0f}" if coin['market_cap'] else "N/A"
 
 cards += f'''
 <div class="crypto-card bg-gray-900/80 backdrop-blur-md rounded-2xl p-6 border border-gray-800 hover:border-blue-500 transition-all hover:scale-105 cursor-pointer shadow-2xl"
 ...
 <style>
 body {{ font-family: 'Inter', sans-serif; background: linear-gradient(135deg, #000000, #05172a, #000000); min-height: 100vh; }}
 .light-mode {{ background: #f1f5f9; }}
 .light-mode .bg-gray-900\\/80 {{ background: rgba(241,245,249,0.8) !important; }}
 ...
 <body class="text-white transition-all duration-500">
 ...
 <h1 class="text-4xl md:text-5xl font-bold bg-gradient-to-r from-blue-400 to-blue-300 bg-clip-text text-transparent">
 TradeScout Pro
 </h1>
 </div>
 <div class="flex items-center space-x-6">
 <input type="text" id="searchInput" placeholder="Search cryptos..." 
 class="px-6 py-3 rounded-full bg-gray-900/70 border border-gray-800 focus:border-blue-500 focus:outline-none text-white w-64">
 <button id="themeToggle" class="p-3 rounded-full bg-gray-900 hover:bg-gray-800 transition text-2xl">
 🌙
 </button>
 </div>
 </header>
 ...
 <div class="bg-gray-900/95 backdrop-blur-xl rounded-3xl p-10 max-w-2xl w-full mx-6 shadow-2xl border border-blue-600/50" onclick="event.stopPropagation()">
 <div class="flex items-center space-x-8 mb-8">
 <img id="modalLogo" src="" class="w-24 h-24 rounded-full shadow-xl">
